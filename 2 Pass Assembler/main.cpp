...
void init()
{
	...
	opc[0] = {"ADD","00000001",1};
	opc[1] = {"ADDI","00000010",5};
	opc[2] = {"CMP","00000011",5};
	opc[3] = {"INC","00000100",1};
	opc[4] = {"JE","00000101",5};
	opc[5] = {"JMP","00000110",5};
	opc[6] = {"LOAD","00000111",5};
	opc[7] = {"LOADI","00001000",1};
	opc[8] = {"MVI","00001001",5};
	opc[9] = {"MOV","00001010",1};
	opc[10] = {"STOP","00001011",1};
	opc[11] = {"STORE","00001100",5};
	opc[12] = {"STORI","00001101",1};
}

int search_opc(string opcode) //Search Machine Opcode Table
{
	int index = -1;
	for(int i = 0;i < 13;i++)
	{
		if(opc[i].name == opcode)
		{
			index = i;
			break;
		}
...
void pass2()
{
	...
	while(infile >> word)
	{
		...
		{
			outfile <<convert(lc)<<" "<<opc[control].binary;
